  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <div className="flex flex-1 md:flex-row md:pt-0 pt-14">
        {/* 데스크톱 사이드바 */}
        <aside className={`border-r bg-background hidden md:block fixed left-0 top-0 h-screen transition-all duration-300 ${isSidebarCollapsed ? "w-16" : "w-64"}`}>
          <div className="flex h-14 items-center border-b px-4 justify-between">
            ...
            <Button variant="ghost" size="icon" className="size-8" onClick={() => setSidebarCollapsed(!isSidebarCollapsed)}>
              {isSidebarCollapsed ? <IconChevronRight className="size-4" /> : <IconChevronLeft className="size-4" />}
          ...
          <nav className="flex-1 overflow-auto py-4 h-[calc(100vh-8rem)]">
            <div className="flex flex-col gap-1 px-2">
              {navItems.map((item) => (
                <Button key={item.href} asChild variant={pathname.startsWith(item.href) ? "secondary" : "ghost"} className={`justify-start ${isSidebarCollapsed ? "px-2" : ""}`}>
                  <Link href={item.href}>
                    {item.icon}
        ...
        {/* 메인 콘텐츠 */}
        <main className="flex-1 flex flex-col" style={{ marginLeft: mainMargin }}>
          {/* 데스크톱 헤더 */}
          <header className="bg-background fixed top-0 right-0 z-30 hidden h-14 items-center gap-4 border-b px-6 md:flex" style={{ left: mainMargin }}>
            <div className="flex-1">
              <h1 className="text-lg font-semibold">{navItems.find((item) => pathname.startsWith(item.href))?.title || "Dashboard"}</h1>
            </div>
